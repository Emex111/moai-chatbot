export default async function handler(req, res) {
  ...
  if (!messages || !Array.isArray(messages)) {
    return res.status(400).json({ error: 'The stone sees no messages.' });
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({
        error: `The stone is cracked: ${data.error?.message || 'Unknown error'}`
      });
    }

    const reply = data?.candidates?.[0]?.content?.parts?.[0]?.text || 'The stone remains silent.';
    return res.status(200).json({ reply });

  } catch (error) {
    console.error("Vercel Function Crash:", error);
    return res.status(500).json({ error: 'The island has sunk into the sea.' });
  }
}